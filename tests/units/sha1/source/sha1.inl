...
bool Sha1()
{
...

	{
		...
		{
			std::array<uint8_t, 20> data{ 193, 34, 82, 206, 218, 139, 232, 153, 77, 95, 160, 41, 10, 71, 35, 28, 29, 22,
				170, 227 };
		...
		{
			std::array<uint8_t, 20> data{ 50, 209, 12, 123, 140, 249, 101, 112, 202, 4, 206, 55, 242, 161, 157, 132, 36,
				13, 58, 137 };
		...
		{
			std::array<uint8_t, 20> data{ 118, 28, 69, 123, 247, 59, 20, 210, 126, 158, 146, 101, 196, 111, 75, 77, 218,
				17, 249, 64 };
		...
		{
			std::array<uint8_t, 20> data{ 1, 179, 7, 172, 186, 79, 84, 245, 90, 175, 195, 59, 176, 107, 187, 246, 202,
				128, 62, 154 };
		...
		{
			std::array<uint8_t, 20> data{ 47, 212, 225, 198, 122, 45, 40, 252, 237, 132, 158, 225, 187, 118, 231, 57,
				27, 147, 235, 18 };
		...
		{
			std::array<uint8_t, 20> data{ 64, 141, 148, 56, 66, 22, 248, 144, 255, 122, 12, 53, 40, 232, 190, 209, 224,
				176, 22, 33 };
			RETURN_IF_FALSE(t.Assert(getSha1("The quick brown fox jumps over the lazy dog."),
				std::span<const uint8_t>(data.data(), 20), "Sha1('quick brown fox.') should be correct"));
		}
		...
		{
			std::array<uint8_t, 20> data{ 132, 152, 62, 68, 28, 59, 210, 110, 186, 174, 74, 161, 249, 81, 41, 229, 229,
				70, 112, 241 };
...

	{
		...
		const auto chunked{ shaChunked.Final() };

		RETURN_IF_FALSE(t.Assert(chunked, single, "Chunked update digits should match single update digits"));
	}

	{
		...
		for (size_t len : { 55u, 56u, 57u, 63u, 64u, 65u }) {
			...
			RETURN_IF_FALSE(t.Assert(chunked, single, "Boundary length chunked vs single digits should match"));
		}
	}
