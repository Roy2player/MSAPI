 ...
 */
FORCE_INLINE [[nodiscard]] bool Authorization();
...
FORCE_INLINE AccountTestData::AccountTestData(
...
FORCE_INLINE [[nodiscard]] bool Authorization()
{
	LOG_INFO("MSAPI UNIT TEST Authorization");
	MSAPI::Test::Test t;
	...
	const auto tryAccess{ [&t, &mod, &checkAccountLogs](
		...
		RETURN_IF_FALSE(t.Assert(mod.IsAccessGranted(current.connectionId, static_cast<G>(gradeToCheck)),
			expectedAccess, std::format("Access is granted to {}, to account {}", gradeToCheck, current.login)));
	...
	const auto checkLogonWithInvalidPassword{ [&t, &mod, &error, &checkAccountLogs](auto& current,
		...
		RETURN_IF_FALSE(checkAccountLogs(*current, current->logsCount - 1,
			std::format(
				"Failed logon attempt at XXXX-XX-XX XX:XX:XX.XXXXXXXXX, connection id {}, reason: Invalid login "
				"or password",
		...
		for (auto current{ begin }; current != end; ++current) {
			if (!current->shouldRegister || current->isDeleted) {
				RETURN_IF_FALSE(t.Assert(
					...
					std::format(
						"Logon connection id {} with account login: '{}'", current->connectionId, current->login)));
			...
			if (!current->isActivated) {
				RETURN_IF_FALSE(
					t.Assert(mod.LogonConnection(current->connectionId, current->login, current->password, error),
				...
				RETURN_IF_FALSE(checkAccountLogs(*current, current->logsCount - 1,
					std::format(
						"Failed logon attempt at XXXX-XX-XX XX:XX:XX.XXXXXXXXX, connection id {}, reason: Account "
						"is not activated",
						current->connectionId)));
			}
			error.clear();
...
#define TMP_MSAPI_UNIT_TEST_AUTHORIZATION_CHANGE_NEW_GRADE_COUNTER                                                     \
	...
	if (end - begin < 4) [[unlikely]] {                                                                                \
		...
		for (auto current{ begin }; current != end; ++current) {
			...
			if (current->reLogon) {
				RETURN_IF_FALSE(
					t.Assert(mod.LogonConnection(current->connectionId, current->login, current->password, error), true,
						std::format(
							"Logon connection id {} with account login: '{}'", current->connectionId, current->login)));
	...
	for (size_t index{}; index < testAccounts.size(); index += 20) {
		...
		RETURN_IF_FALSE(t.Assert(mod.LogonConnection(current.connectionId, current.login, current.password, error),
			false, std::format("Logon connection id {} with deleted account", current.connectionId)));
		RETURN_IF_FALSE(t.Assert(error, "Invalid login or password",
			std::format("Check error message for logon attempt on connection id {} with deleted account",
				current.connectionId)));
		error.clear();
	...
	for (auto& current : testAccounts) {
		...
		RETURN_IF_FALSE(t.Assert(mod.LogonConnection(current.connectionId, current.login, current.password, error),
			true, std::format("Logon connection id {}", current.connectionId)));
		RETURN_IF_FALSE(t.Assert(error, "", "Check error message for login modification"));
		current.lastActivity = Timer{};
		current.isLoggedOn = true;
		++current.logsCount;
	...
	for (auto& current : testAccounts) {
		...
		RETURN_IF_FALSE(t.Assert(mod.LogonConnection(current.connectionId, current.login, current.password, error),
			false, std::format("Logon already logged-on connection id {}", current.connectionId)));
		RETURN_IF_FALSE(t.Assert(error, "", "Check empty error message for successful logon"));
		current.lastActivity = Timer{};
		++current.logsCount;
		RETURN_IF_FALSE(checkAccountLogs(current, current.logsCount - 1,
	...
	for (size_t index{ invalidAccountsSize }; index < testAccounts.size(); index += 7) {
		...
		RETURN_IF_FALSE(t.Assert(
			...
			true,
			std::format("Account {} logout from connection id {}", firstAccount.login, firstAccount.connectionId)));
		--activeConnections;
		RETURN_IF_FALSE(t.Assert(
			mod.GetLogonConnectionsSize(), activeConnections, "Expected number of active connections in module"));
		...
		RETURN_IF_FALSE(checkAccountLogs(firstAccount, firstAccount.logsCount - 2,
			std::format("Logout due to deletion at XXXX-XX-XX XX:XX:XX.XXXXXXXXX, connection id {}",
		...
		RETURN_IF_FALSE(t.Assert(
			...
			true,
			std::format("Account {} logout from connection id {}", secondAccount.login, secondAccount.connectionId)));
		--activeConnections;
		RETURN_IF_FALSE(t.Assert(
			mod.GetLogonConnectionsSize(), activeConnections, "Expected number of active connections in module"));
		...
		RETURN_IF_FALSE(t.Assert(
			...
			std::format(
				"Logon connection id {} with account login: '{}'", firstAccount.connectionId, secondAccount.login)));
		RETURN_IF_FALSE(t.Assert(error, "", "Check empty error message for successful logon"));
	...
	for (auto& current : testAccounts) {
		...
		if (current.isLoggedOn) {
			...
			++current.logsCount;
			RETURN_IF_FALSE(checkAccountLogs(current, current.logsCount - 1,
				std::format("Logout due to module stop at XXXX-XX-XX XX:XX:XX.XXXXXXXXX, connection id {}",
					current.connectionId)));
			--activeConnections;
	...
	for (auto& current : testAccounts) {
		...
		if (!current.blockedTill.Empty()) {
			...
			RETURN_IF_FALSE(checkAccountLogs(current, current.logsCount - 1,
				std::format(
					"Failed logon attempt at XXXX-XX-XX XX:XX:XX.XXXXXXXXX, connection id {}, reason: Account is "
					"blocked till {}",
