...
#ifndef MSAPI_INTEGRATION_TEST_WEB_SOCKET_PROTOCOL_INL
#define MSAPI_INTEGRATION_TEST_WEB_SOCKET_PROTOCOL_INL
...
FORCE_INLINE [[nodiscard]] bool WebSocketProtocol()
{
	LOG_INFO("MSAPI INTEGRATION TEST: Web socket protocol");
	...
	// Server
	auto serverPtr{ MSAPI::Daemon<Node>::Create("Server") };
	...
	MSAPI::Test::Protocol::WebSocket::Observer serverObserver{ server };
	...
	// 1.1. Check validity condition of websocket data
	{
		...
		for (const auto& condition : conditions) {
			MSAPI::Test::Protocol::WebSocket::Observer::SetDataCondition(
				data, condition.opcode, condition.isMasked, condition.headerSize, condition.payloadSize);
			RETURN_IF_FALSE(t.Assert(data.IsValid(), condition.isValid, "Expected data validity state"));
		}
	}

	const auto checkMessage{ [&t](MSAPI::Protocol::WebSocket::Data& message, const bool isFinal, const bool rsv1,
		...
		RETURN_IF_FALSE(t.Assert(message.IsFinal(), isFinal, std::format("Message is final {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.IsRsv1(), rsv1, std::format("Message RSV1 {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.IsRsv2(), rsv2, std::format("Message RSV2 {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.IsRsv3(), rsv3, std::format("Message RSV3 {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.GetOpcode(), opcode, std::format("Message opcode {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.IsMasked(), isMasked, std::format("Message is masked {}", clientPortStr)));
		RETURN_IF_FALSE(
			t.Assert(message.GetHeaderSize(), headerSize, std::format("Message header size {}", clientPortStr)));
		RETURN_IF_FALSE(
			t.Assert(message.GetPayloadSize(), payloadSize, std::format("Message payload size {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.GetBufferSize(), static_cast<size_t>(headerSize) + payloadSize,
			std::format("Message buffer size {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(message.IsValid(), true, std::format("Message is valid {}", clientPortStr)));
		...
		RETURN_IF_FALSE(t.Assert(message.ToString(),
		...
		if (isMasked) {
			if (payloadSize == 0) {
				RETURN_IF_FALSE(t.Assert(maskingKey != 0, true, std::format("Masking key {}", clientPortStr)));
				...
				if (payloadSize < 5 && message.GetPayload() == payload) {
					MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(message);
					if (message.GetPayload() != payload) {
						RETURN_IF_FALSE(t.Assert(message.GetPayload(), payload,
							std::format("Masked message payload content {}", clientPortStr)));
					}
				}
				else {
					RETURN_IF_FALSE(t.Assert(message.GetPayload() != payload, true,
						std::format("Masked message payload content {}", clientPortStr)));
				}
				RETURN_IF_FALSE(t.Assert(maskingKey != 0, true, std::format("Masking key {}", clientPortStr)));
				MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(message);
			...
			RETURN_IF_FALSE(
				t.Assert(message.GetPayload(), payload, std::format("Message payload content {}", clientPortStr)));
		}

		return true;
	} };

	const auto testMergePayload{ [&checkMessage, &t, payloadSpan](const std::span<const uint8_t> payloadTo,
		...
		if (isMasked && !payloadTo.empty() && payloadTo.size() % sizeof(uint32_t) != 0) {
			RETURN_IF_FALSE(t.Assert(true, false,
	...
	// 1.2. Initial state of WebSocket Handler on server
	RETURN_IF_FALSE(t.Assert(serverObserver.GetSizeOfFragmentedDataConnections(), 0,
		"Initial size of fragmented data connections on server"));
	RETURN_IF_FALSE(t.Assert(serverObserver.GetSizeOfFragmentedDataTimerToConnection(), 0,
		"Initial size of fragmented data timer to connection on server"));
	RETURN_IF_FALSE(
		t.Assert(serverObserver.GetStoredFragmentedDataSize(), 0., "Initial stored fragmented data size on server"));
	RETURN_IF_FALSE(t.Assert(server.GetFragmentedDataLimit(), 10., "Default fragmented data limit is correct"));
	RETURN_IF_FALSE(t.Assert(
		server.SetFragmentedDataLimit(-1.), false, "Setting fragmented data limit to negative value should fail"));
	RETURN_IF_FALSE(
		t.Assert(server.GetFragmentedDataLimit(), 10., "Fragmented data limit should remain 10 after failed set"));
	RETURN_IF_FALSE(
		t.Assert(server.SetFragmentedDataLimit(0.), true, "Setting fragmented data limit to 0 should succeed"));
	RETURN_IF_FALSE(
		t.Assert(server.GetFragmentedDataLimit(), 0., "Fragmented data limit should be 0 after successful set"));
	RETURN_IF_FALSE(
		t.Assert(server.SetFragmentedDataLimit(5.), true, "Setting fragmented data limit to 5 should succeed"));
	RETURN_IF_FALSE(
		t.Assert(server.GetFragmentedDataLimit(), 5., "Fragmented data limit should be 5 after successful set"));
	...
	{
		MSAPI::Protocol::WebSocket::Data data;
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 0, "Masking key is expected"));
		RETURN_IF_FALSE(
	...
	// 1.7. Try to create SplitGenerator with empty payload
	{
		...
		RETURN_IF_FALSE(
			t.Assert(generator.Get(), false, "SplitGenerator with empty payload should not have fragments"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 0, "Masking key is expected"));
	}
	{
		MSAPI::Protocol::WebSocket::Data data{ {}, MSAPI::Protocol::WebSocket::Data::Opcode::Text, 1234 };
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 1234, "Masking key is expected"));
		MSAPI::Protocol::WebSocket::Data::SplitGenerator generator{ data, std::span<const uint8_t>{}, 10, true };
		RETURN_IF_FALSE(
			t.Assert(generator.Get(), false, "SplitGenerator with empty payload should not have fragments"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey() == 1234, true, "Masking key is expected"));
	}
	...
	// 1.8. Try to create SplitGenerator with payload less than minimum
	{
		...
		RETURN_IF_FALSE(
			t.Assert(generator.Get(), false, "SplitGenerator with too small payload should not have fragments"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 0, "Masking key is expected"));
	}
	{
		MSAPI::Protocol::WebSocket::Data data{ {}, MSAPI::Protocol::WebSocket::Data::Opcode::Text, 23423 };
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 23423, "Masking key is expected"));
		MSAPI::Protocol::WebSocket::Data::SplitGenerator generator{ data, payloadSpan.subspan(0, 3), 10, true };
		RETURN_IF_FALSE(
			t.Assert(generator.Get(), false, "SplitGenerator with too small payload should not have fragments"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey() == 23423, true, "Masking key is expected"));
	}
	{
		MSAPI::Protocol::WebSocket::Data data{ {}, MSAPI::Protocol::WebSocket::Data::Opcode::Text };
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 0, "Masking key is expected"));
		...
		RETURN_IF_FALSE(
			t.Assert(generator.Get(), false, "SplitGenerator with too small payload should not have fragments"));
		RETURN_IF_FALSE(t.Assert(data.IsMasked(), false, "Is masked is expected"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 0, "Masking key is expected"));
	}
	{
		MSAPI::Protocol::WebSocket::Data data{ {}, MSAPI::Protocol::WebSocket::Data::Opcode::Text, 23423 };
		...
		RETURN_IF_FALSE(
			t.Assert(generator.Get(), false, "SplitGenerator with too small payload should not have fragments"));
		RETURN_IF_FALSE(t.Assert(data.IsMasked(), true, "Is masked is expected"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 23423, "Masking key is expected"));
	}
	...
	// 1.9. Try to create SplitGenerator with zero step
	{
		...
		RETURN_IF_FALSE(t.Assert(generator.Get(), true, "SplitGenerator get with zero"));
	...
	}
	{
		...
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 6542342, "Masking key is expected"));
		MSAPI::Protocol::WebSocket::Data::SplitGenerator generator{ data, payloadSpan.subspan(0, 4), 0, true };
		RETURN_IF_FALSE(t.Assert(generator.Get(), true, "SplitGenerator get with zero"));
		RETURN_IF_FALSE(checkMessage(data, false, false, false, false, MSAPI::Protocol::WebSocket::Data::Opcode::Text,
			true, payloadSpan.subspan(0, 1), ""));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey() != 6542342, true, "Masking key is expected"));
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey() != 0, true, "Masking key is expected"));
	}
	{
		MSAPI::Protocol::WebSocket::Data data{ {}, MSAPI::Protocol::WebSocket::Data::Opcode::Text };
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 0, "Masking key is expected"));
	...
	}
	{
		...
		RETURN_IF_FALSE(t.Assert(data.GetMaskingKey(), 543345, "Masking key is expected"));
		MSAPI::Protocol::WebSocket::Data::SplitGenerator generator{ data, payloadSpan.subspan(0, 4), 0 };
		RETURN_IF_FALSE(t.Assert(generator.Get(), true, "SplitGenerator get with zero"));
		RETURN_IF_FALSE(checkMessage(data, false, false, false, false, MSAPI::Protocol::WebSocket::Data::Opcode::Text,
			false, payloadSpan.subspan(0, 1), ""));
	...
	// 1.10. Try to create SplitGenerator with step greater than payload size
	{
		...
		MSAPI::Protocol::WebSocket::Data::SplitGenerator generator{ data, payloadSpan.subspan(0, 65535 * 4 + 1),
			65535 * 4 + 2 };
		RETURN_IF_FALSE(t.Assert(generator.Get(), true, "SplitGenerator get with step greater than payload size"));
		RETURN_IF_FALSE(checkMessage(data, false, false, false, false, MSAPI::Protocol::WebSocket::Data::Opcode::Text,
	...
	// 1.11. Try to create split generator for different opcodes
	{
		...
		for (const auto testData : opcodes) {
			...
			RETURN_IF_FALSE(t.Assert(generator.Get(), testData.expectedResult,
...
#define TMP_MSAPI_PROTOCOL_WEBSOCKET_DATA_SPLIT_GENERATOR_CONSTRUCT_WITH_PAYLOAD_TYPE(type)                            \
	{                                                                                                                  \
		...
		RETURN_IF_FALSE(t.Assert(generator.Get(), true, "Successfully get from split generator"));                     \
		RETURN_IF_FALSE(checkMessage(data, false, false, false, false, MSAPI::Protocol::WebSocket::Data::Opcode::Text, \
...
#undef TMP_MSAPI_PROTOCOL_WEBSOCKET_DATA_CLOSE_CONSTRUCT_WITH_PAYLOAD_TYPE
	...
	// 1.15. Test expected header size function
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(0, false), 2,
		"Expected header size for unmasked zero payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(125, false), 2,
		"Expected header size for unmasked 125 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(0, true), 6,
		"Expected header size for masked zero payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(125, true), 6,
		"Expected header size for masked 125 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(126, false), 4,
		"Expected header size for unmasked 126 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(65535, false), 4,
		"Expected header size for unmasked 65535 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(126, true), 8,
		"Expected header size for masked 126 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(65535, true), 8,
		"Expected header size for masked 65535 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(65536, false), 10,
		"Expected header size for unmasked 65536 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(3000000, false), 10,
		"Expected header size for unmasked 3000000 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(65536, true), 14,
		"Expected header size for masked 65536 payload"));
	RETURN_IF_FALSE(t.Assert(MSAPI::Protocol::WebSocket::Data::GetExpectedHeaderSize(3000000, true), 14,
		"Expected header size for masked 3000000 payload"));

	struct ClientDaemon {
		std::unique_ptr<MSAPI::Daemon<Node>> ptr;
		std::string portStr;
		...
		FORCE_INLINE ClientDaemon(std::unique_ptr<MSAPI::Daemon<Node>>&& daemonPtr)
	...
	std::vector<ClientDaemon> clientDaemons;

	const auto doTest{ [&serverPtr, &server, &t, payloadSpan, &checkMessage, &serverObserver, &clientDaemons,
						   closeCodeInBuffer](const bool isMasked) -> bool {
		auto& clientDaemon{ clientDaemons.emplace_back(ClientDaemon{ MSAPI::Daemon<Node>::Create("Client") }) };
		if (clientDaemon.ptr == nullptr) {
			return false;
		}
		auto& client{ clientDaemon.ptr->GetApp() };
		clientDaemon.clientToServerConnectionData
			= client.OpenConnection(INADDR_LOOPBACK, serverPtr->GetPort(), /*doReconnection=*/false);
		RETURN_IF_FALSE((clientDaemon.clientToServerConnectionData != nullptr));
		const auto& CTSCD{ clientDaemon.clientToServerConnectionData };
		client.HandleRunRequest();
		const std::string& clientPortStr{ clientDaemon.portStr };

		MSAPI::Test::Protocol::WebSocket::Observer clientObserver{ client };
		RETURN_IF_FALSE(t.Assert(clientObserver.GetSizeOfFragmentedDataConnections(), 0,
		...
		RETURN_IF_FALSE(t.Wait(
			50000,
			[&server, &clientPortStr, &clientDaemon]() {
				clientDaemon.serverToClientConnectionData = server.DetectConnection(clientPortStr);
				return clientDaemon.serverToClientConnectionData != nullptr;
			},
			true,
			std::format("Server detected connection for WebSocket handshake request from client {}", clientPortStr)));

		const auto& STCCD{ clientDaemon.serverToClientConnectionData };
		const auto* serverHttpData{ server.GetHttpData(STCCD) };
		uint64_t expectedServerHttpDataSize{};
		RETURN_IF_FALSE(t.Assert(
			serverHttpData != nullptr, true, std::format("Server has HTTP data for client {}", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(serverHttpData->size(), ++expectedServerHttpDataSize,
			std::format("Server has {} HTTP data messages for client {}", serverHttpData->size(), clientPortStr)));
		RETURN_IF_FALSE(t.Assert(serverHttpData->back().ToString(), wsHandshakeRequestInvalid,
		...
		uint64_t expectedClientHttpDataSize{};
		...
		RETURN_IF_FALSE(t.Wait(
			...
			true, std::format("Client {} has HTTP data", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(clientHttpData->size(), ++expectedClientHttpDataSize,
			std::format("Client {} has {} HTTP data messages", clientHttpData->size(), clientPortStr)));
		RETURN_IF_FALSE(t.Assert(clientHttpData->back().ToString(), wsHandshakeResponseInvalid,
		...
		RETURN_IF_FALSE(t.Wait(
			50000, [&serverHttpData, &clientPortStr]() { return serverHttpData->size(); }, expectedServerHttpDataSize,
			std::format("Server has expected number HTTP data messages for client", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(serverHttpData->back().ToString(), wsHandshakeRequest,
			std::format("Server got correct WebSocket handshake request from client {}", clientPortStr)));

		++expectedClientHttpDataSize;
		RETURN_IF_FALSE(t.Wait(
			50000, [&clientHttpData, &clientPortStr]() { return clientHttpData->size(); }, expectedClientHttpDataSize,
			std::format("Client {} has expected number HTTP data messages", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(clientHttpData->back().ToString(), wsHandshakeResponse,
			std::format("Client {} got correct WebSocket handshake response", clientPortStr)));

		RETURN_IF_FALSE(t.Assert(clientObserver.GetSizeOfFragmentedDataConnections(), 0,
			std::format("Size of fragmented data connections on client {} after handshake", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(serverObserver.HasConnectionFragmentedData(STCCD->GetConnectionId()), false,
			std::format("Server does not have fragmented data with client {} after handshake", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(serverObserver.GetLastFragmentedDataTimer(STCCD->GetConnectionId()), MSAPI::Timer{ 0 },
			std::format("Server does not have timestamp of last fragmented data with client {} after handshake",
				clientPortStr)));
		RETURN_IF_FALSE(t.Assert(clientObserver.HasConnectionFragmentedData(CTSCD->GetConnectionId()), false,
			std::format("Client {} does not have fragmented data with server after handshake", clientPortStr)));

		RETURN_IF_FALSE(t.Assert(client.GetWebSocketData(CTSCD), nullptr,
			std::format("Client {} does not have WebSocket yet", clientPortStr)));
		RETURN_IF_FALSE(t.Assert(server.GetWebSocketData(STCCD), nullptr,
		...
		const auto sendAndCheck{ [&t, &serverWebSocketData, &expectedServerWebSocketDataSize, &server, &STCCD,
			...
			if (expectedClientWebSocketDataSize > 0) {
				RETURN_IF_FALSE(t.Wait(
					50000,
					[&clientWebSocketData, &client, &CTSCD]() {
						clientWebSocketData = client.GetWebSocketData(CTSCD);
						if (clientWebSocketData == nullptr) [[unlikely]] {
							return uint64_t(0);
						}

						return clientWebSocketData->size();
					},
					expectedClientWebSocketDataSize,
			...
			RETURN_IF_FALSE(t.Wait(
				50000,
				[&serverWebSocketData, &server, &STCCD]() {
					serverWebSocketData = server.GetWebSocketData(STCCD);
					if (serverWebSocketData == nullptr) [[unlikely]] {
						return uint64_t(0);
					}

					return serverWebSocketData->size();
				},
				expectedServerWebSocketDataSize,
			...
			if (opcode != MSAPI::Protocol::WebSocket::Data::Opcode::Close) {
				if (opcode == MSAPI::Protocol::WebSocket::Data::Opcode::Ping) {
					...
					RETURN_IF_FALSE(t.Assert(clientLastMessage.GetMaskingKey() == 0, message.GetMaskingKey() != 0,
					...
					if (message.IsMasked()) {
						...
						MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(
					...
					if (message.IsMasked()) {
						if (!message.GetPayload().empty()) {
							RETURN_IF_FALSE(t.Assert(message.GetPayload() != lastServerMessage.GetPayload(), true,
								std::format("Masked message payload content {}", clientPortStr)));
							// Collected message always unmasked, checkMessage expect masked payload for masked message
							MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(message);
						}
					}
					RETURN_IF_FALSE(t.Assert(lastServerMessage, message,
		...
		const auto getMessageWithPayload{ [&t, &clientPortStr,
		...
		const auto doSendAndCheckWithPayload{ [&checkMessage, &sendAndCheck, &getMessageWithPayload, &clientPortStr,
												  &t]<MSAPI::Protocol::WebSocket::Data::Opcode O>(const bool isFinal,
												  const bool rsv1, const bool rsv2, const bool rsv3,
												  const bool isMasked, const std::span<const uint8_t> payload) -> bool {
			if constexpr (O == MSAPI::Protocol::WebSocket::Data::Opcode::Close) {
				if (payload.size() < 2) {
					RETURN_IF_FALSE(t.Assert(payload.empty(), true,
						std::format(
							"Close message payload must be empty if it does not contain status code, client: {}",
							clientPortStr)));
				}
		...
		const auto waitForFragmentedData{ [&t, &serverObserver, &STCCD, &clientPortStr](
				...
				"Client {} does not have fragmented data with server before fragmented messages", clientPortStr)));
		{
			...
			for (auto [payloadSize, step] : continuationTestData) {
				...
				while (generator.Get()) {
					...
					if (sendedFragments != expectedFragments) {
						...
						if (isMasked) {
							...
							RETURN_IF_FALSE(t.Assert(previousMaskingKey != newMaskingKey, true,
						...
						if (sendedFragments == 1) {
							...
							RETURN_IF_FALSE(t.Assert(clientObserver.GetSizeOfFragmentedDataConnections(), 0,
								std::format("Size of fragmented data connections on client {} after fragmented message",
									clientPortStr)));
							RETURN_IF_FALSE(
								t.Assert(clientObserver.HasConnectionFragmentedData(CTSCD->GetConnectionId()), false,
									std::format(
				...
				RETURN_IF_FALSE(t.Assert(sendedFragments, expectedFragments,
					std::format("All fragments were generated for payload size {} and step {}, client {}", payloadSize,
						step, clientPortStr)));

				++expectedServerWebSocketDataSize;
				RETURN_IF_FALSE(t.Wait(
					50000, [&serverWebSocketData, &STCCD]() { return serverWebSocketData->size(); },
					expectedServerWebSocketDataSize,
				...
				if (lastServerWebSocketData.IsMasked()) {
					// Collected message always unmasked, checkMessage expect masked payload for masked message
					MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(lastServerWebSocketData);
				}
				...
				RETURN_IF_FALSE(t.Assert(serverObserver.HasConnectionFragmentedData(STCCD->GetConnectionId()), false,
					std::format(
						...
						clientPortStr)));
				RETURN_IF_FALSE(
					t.Assert(serverObserver.GetLastFragmentedDataTimer(STCCD->GetConnectionId()), MSAPI::Timer{ 0 },
				...
				auto& firstByteOfData{ MSAPI::Test::Protocol::WebSocket::Observer::GetFirstByte(data) };
		...
		// 2.22. Overwrite fragmented message with new initial message
		{
			...
			++expectedServerWebSocketDataSize;
			RETURN_IF_FALSE(t.Wait(
				50000, [&serverWebSocketData]() { return serverWebSocketData->size(); },
				expectedServerWebSocketDataSize,
				std::format("Server has WebSocket data with {} messages for client {}", expectedServerWebSocketDataSize,
					clientPortStr)));
			...
			if (isMasked) {
				// Collected message always unmasked, checkMessage expect masked payload for masked message
				MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(lastServerWebSocketData1);
			}
			...
			RETURN_IF_FALSE(checkMessage(lastServerWebSocketData1, true, false, false, false,
				...
				clientPortStr));
			RETURN_IF_FALSE(t.Assert(serverObserver.HasConnectionFragmentedData(STCCD->GetConnectionId()), true,
				std::format(
			...
			if (isMasked) {
				// Collected message always unmasked, checkMessage expect masked payload for masked message
				MSAPI::Test::Protocol::WebSocket::Observer::ApplyMaskToData(lastServerWebSocketData2);
			}
	...
	RETURN_IF_FALSE(
		t.Assert(serverObserver.GetStoredFragmentedDataSize(), 0., "Final stored fragmented data size on server"));
	...
	// 3.2. Check fragmented data storage purging due to limits with one connection scenarios
	{
		...
		auto& clientDaemon{ clientDaemons[0] };
		auto& client{ clientDaemon.ptr->GetApp() };
		const std::string& clientPortStr{ clientDaemon.portStr };
		const auto& CTSCD{ clientDaemon.clientToServerConnectionData };
		const auto& STCCD{ clientDaemon.serverToClientConnectionData };
		...
		if (client.GetWebSocketData(CTSCD) == nullptr) [[unlikely]] {
			RETURN_IF_FALSE(
				t.Assert(true, false, std::format("Client {} does not have web socket data", clientPortStr)));
		}
		...
		// 3.2.3. Send fragmented message with total payload size greater than fragmented data limit
		{
			...
			RETURN_IF_FALSE(
				t.Assert(serverObserver.GetLastFragmentedDataTimer(STCCD->GetConnectionId()) != MSAPI::Timer{ 0 }, true,
					std::format("Server has timestamp of last fragmented data with client {} after fragmented message",
						clientPortStr)));
			RETURN_IF_FALSE(t.Assert(serverObserver.GetStoredFragmentedDataSize(),
			...
			RETURN_IF_FALSE(
				t.Assert(serverObserver.GetLastFragmentedDataTimer(STCCD->GetConnectionId()), MSAPI::Timer{ 0 },
					std::format("Server does not have timestamp of last fragmented data with client {} after sending "
						...
						clientPortStr)));
			RETURN_IF_FALSE(t.Assert(clientWebSocketDataSize, client.GetWebSocketData(CTSCD)->size(),
				std::format(
					...
					clientPortStr)));
			RETURN_IF_FALSE(t.Assert(serverWebSocketDataSize, server.GetWebSocketData(STCCD)->size(),
			...
			RETURN_IF_FALSE(t.Assert(serverObserver.GetSizeOfFragmentedDataTimerToConnection(), 0,
				std::format("Size of fragmented data timer to connection on server after purging fragment data for "
							"client {}",
					clientPortStr)));
			RETURN_IF_FALSE(t.Assert(serverObserver.GetStoredFragmentedDataSize(), 0.,
				std::format(
		...
		// 3.2.5. Check zero fragmented data limit with 1 byte payload messages
		{
			...
			RETURN_IF_FALSE(t.Wait(
				50000, [&server, &STCCD]() { return server.GetWebSocketData(STCCD)->size(); }, serverWebSocketDataSize,
		...
		{
			RETURN_IF_FALSE(
				t.Assert(server.SetFragmentedDataLimit(1.), true, "Setting fragmented data limit to 1 should succeed"));
			RETURN_IF_FALSE(t.Assert(
			...
			RETURN_IF_FALSE(t.Wait(
				...
				true, std::format("Server has fragmented data messages for client {}", clientPortStr)));
			RETURN_IF_FALSE(
				t.Assert(serverObserver.GetLastFragmentedDataTimer(STCCD->GetConnectionId()) != MSAPI::Timer{ 0 }, true,
					std::format("Server has timestamp of last fragmented data with client {}", clientPortStr)));
			RETURN_IF_FALSE(t.Assert(clientWebSocketDataSize, client.GetWebSocketData(CTSCD)->size(),
			...
			RETURN_IF_FALSE(t.Assert(serverObserver.GetSizeOfFragmentedDataConnections(), 1,
				std::format("Size of fragmented data connections on server", clientPortStr)));
			RETURN_IF_FALSE(t.Assert(serverObserver.GetSizeOfFragmentedDataTimerToConnection(), 1,
			...
			RETURN_IF_FALSE(t.Assert(server.SetFragmentedDataLimit(1.00001), true,
				"Setting fragmented data limit to 1.00001 should succeed"));
			RETURN_IF_FALSE(t.Assert(server.GetFragmentedDataLimit(), 1.00001,
			...
			RETURN_IF_FALSE(t.Assert(serverObserver.HasConnectionFragmentedData(STCCD->GetConnectionId()), false,
				std::format("Server does not have fragmented data messages for client {}", clientPortStr)));
			RETURN_IF_FALSE(t.Assert(serverObserver.GetLastFragmentedDataTimer(STCCD->GetConnectionId()),
				MSAPI::Timer{ 0 },
...
#endif // MSAPI_INTEGRATION_TEST_WEB_SOCKET_PROTOCOL_INL