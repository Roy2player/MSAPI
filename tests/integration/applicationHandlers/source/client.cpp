...
void Client::HandleMetadata(const std::shared_ptr<MSAPI::Connection::Data>& connectionData, [[maybe_unused]] const std::string_view metadata)
{
...
void Client::HandleParameters(
	const std::shared_ptr<MSAPI::Connection::Data>& connectionData, [[maybe_unused]] const std::map<size_t, std::variant<standardTypes>>& parameters)
{
