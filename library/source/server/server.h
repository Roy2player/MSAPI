 ...
 * default is 5, minimum is 1.
 * @brief Const parameter 1000004 "Recv buffer size limit" is a limit of buffer for recv function inside connection
 * request handler. Default is 8 megabytes, minimum is 1024 bytes. Will be applied for already allocated buffers.
 * @brief Const parameter 1000005 "Server state" is a state of server.
 * @brief Const parameter 1000006 "Max connections" is a SOMAXCONN number.
 * @brief Const parameter 1000007 "Listening IP" is a IP address of server to listen after starting.
 * @brief Const parameter 1000008 "Listening port" is a port of server to listen after starting.
 *
...
public:
	...
	FORCE_INLINE void Stop() noexcept
	{
		...
		MSAPI::Lock::AtomicRW::Guard<Lock::write> _{ m_closingConnectionsLock };
	 ...
	 * @todo Add unit test.
	 */
	FORCE_INLINE [[nodiscard]] State GetState() const noexcept { return m_state.load(std::memory_order_acquire); }
	...
	template <Connection::Type Type> FORCE_INLINE void RecvLoop(const std::shared_ptr<Connection::Data>& connectionData)
	{
		...
		MSAPI::Lock::AtomicRW::Guard<Lock::read> _{ m_closingConnectionsLock };
...
private:
		...
		const uint32_t ip, const uint16_t port, const bool doReconnection, const uint64_t oldId) noexcept
	{
		...
		std::cout << "--> before check -1" << std::endl;

		pthread_attr_t attr;
		pthread_attr_init(&attr);
		AddPthreadAttributes(&attr);

		std::cout << "--> before check 0" << std::endl;
		...
		if (connectionData != nullptr) [[likely]] {
			std::cout << "--> before check 2" << std::endl;
			if (m_state.load(std::memory_order_acquire) == State::Running) {
				std::cout << "--> before check 3" << std::endl;
				Protocol::Standard::SendActionHello(connectionData->GetConnection());
			}
			std::cout << "--> before check 4" << std::endl;

			return connectionData;
		}

		std::cout << "--> before check 5" << std::endl;
	 ...
	 *
	 * @locking Read lock m_closingConnectionsLock to prevent closing connection while creating pthread.
	...
	{
		MSAPI::Lock::AtomicRW::Guard<Lock::read> guard{ m_closingConnectionsLock };
	 ...
	 * constant false parameter is provided, but that is not recognized by compiler as a reason for branch optimization.
	 *
	 * @param connectionData Connection data structure.
	 * @param doReconnection If reconnection is required for this connection.
	 *
		...
		const bool doReconnection /* separate to make overridable */) noexcept
	{
		...
		case Connection::Type::Manager: {
			...
			if constexpr (HasReconnectionPath == reconnectionIsPossible) {
				...
				if (doReconnection) {
					...
					LOG_WARNING_NEW("And the reconnect is happened! connection data cannot be nullptr {}",
						bool(newConnectionData != nullptr));

					if (newConnectionData != nullptr) {
						LOG_WARNING("It is called!!");
		...
		case Connection::Type::Income: {
...

			{
				...
				LOG_INFO_NEW("Income connection is closed, id: {}, {}:{}. Active connections counter is {}", id, ipStr,
					connectionData->GetPort(), GetConnectionsCount());
			}
		}
			return;
