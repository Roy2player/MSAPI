...
public:
	...
	{
		return m_cipher == 2666999999 && m_bufferSize >= sizeof(uint64_t) * 4 && m_objectHash != 0 && m_streamId != 0;
	}
...
public:
	...
	/**************************
	 * @brief Set snapshot done flag.
	 *
	 * @locking Write lock inside.
	 *
	 * @todo Add unit test.
	 */
	FORCE_INLINE void SetSnapshotDone(const bool value) noexcept
	{
		Lock::AtomicRW::Guard<Lock::write> _{ m_lock };
		m_isSnapshotDone = value;
	}
	...
	{
		Lock::AtomicRW::Guard<Lock::read> _{ m_lock };
		return m_connectionData == nullptr;
	}
...
protected:
	 ...
	 * @todo Add unit test.
	 */
	void SetConnectionData(std::shared_ptr<Connection::Data>&& connectionData)
	{
		{
			Lock::AtomicRW::Guard<Lock::read> _{ m_lock };
		...
		LOG_PROTOCOL_NEW(
			"Stream id: {} is now related with connection id: {}", m_id, m_connectionData->GetConnectionId());
	}

	// To lock once at stream state update parsing
...
struct StreamKey {
	struct Hash {
		...
		{
			return key.streamId
				^ (key.connectionId + 0x9e3779b97f4a7c15ULL + (key.streamId << 6) + (key.streamId >> 2));
		}
	};
...
protected:
	...
	FORCE_INLINE void SetStream(StreamBase* const stream) noexcept
	{
		...
		LOG_PROTOCOL_NEW("Stream:connection id {}:{} is assigned to handler", streamId, connectionId);
	}
	...
	FORCE_INLINE void ResetStream(const StreamKey streamKey, StreamBase* const stream)
	{
		const auto streamId{ stream->GetId() };
		const auto connectionData{ stream->GetConnectionData() };
		...
		LOG_PROTOCOL_NEW("Stream:connection id {}:{} is reassigned to new connection id: {} in handler", streamId,
			streamKey.connectionId, connectionId);
	}
	...
	FORCE_INLINE void CollectStreamState(const StreamKey& streamKey, const StreamStateResponse* const state) noexcept
	{
		...
		StreamBase* stream [[gnu::uninitialized]];
		{
			Lock::AtomicRW::Guard<Lock::write> _{ m_streamKeyToStreamLock };
		...
		switch (state->state) {
		case State::Opened: {
			Lock::AtomicRW::Guard<Lock::write> _{ stream->m_lock };
		...
		case State::Failed:
			...
			HandleStreamFailed(streamKey.streamId, state->issue);
			return;
		case State::Closed:
			// Write lock inside
			stream->SetState(state->state);
...
public:
	 ...
	 * @todo Add unit test.
	 */
	FORCE_INLINE IHandler(const Application* application) noexcept
	...
	FORCE_INLINE void Collect(const Data& data, const uint64_t connectionId, const void* object) noexcept
	{
		...
		LOG_PROTOCOL_NEW("Collect {}", data.ToString());
...
private:
	const static inline uint64_t m_filterObjecthash{ typeid(T).hash_code() };
...
public:
	...
	template <typename T1>
		requires std::is_same_v<T, std::decay_t<T1>>
	FORCE_INLINE [[nodiscard]] uint64_t SetObject(T1&& object) noexcept
	{
		m_objects.emplace_back(std::forward<T1>(object));
...
public:
	...
	FORCE_INLINE ~Stream() noexcept
	{
		...
		LOG_PROTOCOL_NEW("Client removes stream id: {} with state: {}, connection id: {}", m_id, EnumToString(m_state),
			connectionId);

		m_handler.RemoveStream({ m_id, connectionId });
	...
	{
		if (m_connectionData == nullptr) {
			StreamBase::SetConnectionData(std::move(connectionData));
			m_handler.SetStream(this);
			return;
		}

		const auto previousConnectionId{ m_connectionData->GetConnectionId() };
		StreamBase::SetConnectionData(std::move(connectionData));
		m_handler.ResetStream(StreamKey{ m_id, previousConnectionId }, this);
	}
	...
	template <typename F1>
		requires std::is_same_v<std::decay_t<F1>, Filter<F>>
	FORCE_INLINE void SetFilter(F1&& filter) noexcept
	{
		...
		auto& filterValue{ (m_filter = std::forward<F1>(filter)).value() };

		filterValue.SetStreamObjectHash(typeid(T).hash_code());
		filterValue.SetTotalFilterSize(filterValue.GetObjects().size());
		LOG_PROTOCOL_NEW("Client sets: {} for stream id: {}", filterValue.ToString(), m_id);
	}
	...
	FORCE_INLINE [[nodiscard]] bool Open() noexcept
	{
		...
		if (m_connectionData == nullptr) [[unlikely]] {
			LOG_PROTOCOL_NEW("Client tries to open stream id {} without connection", m_id);
			return false;
		}
		...
		if (!m_filter.has_value()) [[unlikely]] {
			LOG_PROTOCOL_NEW("Client tries to open stream id {} without filter", m_id);
			return false;
		}

		if (m_state == State::Opened || m_state == State::Pending) [[unlikely]] {
			LOG_PROTOCOL_NEW("Stream id {} is already in active state {}", m_id, EnumToString(m_state));
			return false;
		}

		const auto& filterValue{ m_filter.value() };
		m_state = State::Pending;
		LOG_PROTOCOL_NEW("Client opens stream id {} with {}", m_id, filterValue.ToString());
	...
	FORCE_INLINE void Close() noexcept
	{
		...
		if (m_connectionData == nullptr) [[unlikely]] {
			LOG_PROTOCOL_NEW("Client tries to close stream id {} without connection", m_id);
			return;
		}

		if (m_state != State::Opened && m_state != State::Pending) [[unlikely]] {
			LOG_PROTOCOL_NEW("Reject attempt to close stream id {} with state {}", m_id, EnumToString(m_state));
			return;
		}
...
private:
	...
	FORCE_INLINE void CloseImpl() noexcept
	{
		...
		// contract_assert(m_connectionData != nullptr, std::format("Connection is not set for stream id {}",
...
public:
	...
	private:
		...
		{
			m_filter = Filter<T>(std::move(filter));
		}
		 ...
		 * @todo Add unit test.
		 */
		FORCE_INLINE [[nodiscard]] Streams& GetStreams() const noexcept { return *m_streams; }
...
private:
	...
	class Streams {
	private:
		std::unordered_map<StreamKey, std::shared_ptr<StreamData>, StreamKey::Hash> m_streamKeyToStreamData;
	...
	public:
		 ...
		 * @todo Add unit test.
		 */
		FORCE_INLINE [[nodiscard]] std::unordered_map<StreamKey, std::shared_ptr<StreamData>, StreamKey::Hash>&
		GetStreams() noexcept
		{
...
private:
	...
	std::unordered_map<StreamKey, std::shared_ptr<StreamData>, StreamKey::Hash> m_streamKeyToStreamData;
	Lock::AtomicRW m_streamKeyToStreamDataLock;
	//! Here can be stored handler, inside streams
	//! Stream data can have reference on it. Is should be constant without additional lock
...
public:
	...
	{
		{
			...
			while (true) {
				...
				if (streamData->IsActive()) {
					...
					(void)Send(streamData->GetConnection(), Data{ begin->first.streamId, hash, size }, &state);
				}
	...
	void Collect(const std::shared_ptr<Connection::Data>& connectionData, const Data& data, const void* object)
	{
		...
		if (!m_application->IsRunning()) [[unlikely]] {
			LOG_PROTOCOL_NEW("Application state is not Running, collect {}, connection id: {}", data.ToString(),
				streamKey.connectionId);
			return;
		}

		LOG_PROTOCOL_NEW("Collect {}, connection id: {}", data.ToString(), streamKey.connectionId);
		...
		if (typeid(Filter<T>).hash_code() == dataHash) {
			...
			const auto isEmptyFilter{ filter->Empty() };

			if (streamType == Type::SnapshotAndLive || !isEmptyFilter) {
				std::pair<typename std::decay_t<decltype(m_streamKeyToStreamData)>::iterator, bool> result;
				{
					Lock::AtomicRW::Guard<Lock::write> _{ m_streamKeyToStreamDataLock };
			...
			if (!isEmptyFilter) {
				Lock::AtomicRW::Guard<Lock::write> _{ streamData->GetLock() };
				streamData->template SetFilter<T>(*filter);
				LOG_PROTOCOL_NEW("Waiting filter objects for new stream: {}", streamData->ToString());
				return;
			}

			{
				Lock::AtomicRW::Guard<Lock::write> _{ streamData->GetLock() };
				streamData->template SetFilter<T>(*filter);
				LOG_PROTOCOL_NEW("Instantly open new stream: {}", streamData->ToString());
			}
			...
			//! Access is safe as this is const member
			//! streams->HandleNewStreamOpen(*streamData);
		...
		if (typeid(T).hash_code() == dataHash) [[likely]] {
			...
			if (isActive) {
				LOG_PROTOCOL_NEW("Got last filter object and open stream id: {}, connection id: {}", streamKey.streamId,
					streamKey.connectionId);
			...
			LOG_PROTOCOL_NEW(
				"Got filter object for stream id: {}, connection id: {}", streamKey.streamId, streamKey.connectionId);
			return;
		}

		LOG_ERROR_NEW("Unknown hash has been reserved: {}, stream id: {}, connection id: {}", dataHash,
			streamKey.streamId, streamKey.connectionId);
	...
	[[nodiscard]] bool SendObjectsToStream(StreamData& streamData, const T<S>& objects,
		const std::function<bool(const FilterBase* filter, const S& object)>& filterPredicate)
	{
	...
	template <typename T>
		requires std::is_class_v<T>
	[[nodiscard]] bool SendObjectToStream(StreamData& streamData, const T& object,
		const std::function<bool(const FilterBase* filter, const T& object)>& filterPredicate)
	{
		const FilterBase* filter [[gnu::uninitialized]];
		auto& connection{ streamData.GetConnection() };
		Streams* streams [[gnu::uninitialized]];
		...
		if (filter->GetTotalFilterSize() == 0) {
			if (!Send(connection, { streamData.GetStreamId(), typeid(T).hash_code(), sizeof(T) }, &object))
				[[unlikely]] {
				const StreamKey streamKey{ streamData.GetStreamId(), connection.GetId() };
				// Locks inside
				streams->RemoveStream(streamKey);
	...
	template <typename T>
		requires std::is_class_v<T>
	void SendNewObject(
		const T& object, const std::function<bool(const FilterBase* filter, const T& object)> filterPredicate)
	{
		std::shared_ptr<Streams> streams;
