 ...
 * @todo Add unit test.
 */
FORCE_INLINE void SendFailed(
	uint64_t id, Connection& connection, std::string_view error);
	...
	const uint64_t id, const uint64_t hash, const std::shared_ptr<Connection::Data>& connectionData, Json&& json)
{
	if (json.GetValueType<bool>("interrupt") != nullptr) {
		...
		if (eventsData->EraseEvent(id)) {
			LOG_PROTOCOL_NEW(
				"Event for connection id {} with id {} hash {} is interrupted", connectionId, id, hash);
			return;
		}

		LOG_PROTOCOL_NEW(
			"Event for connection id {} with id {} hash {} is not found", connectionId, id, hash);
		return;
	}
	...
	if (!m_authorization.IsAccessGranted(connectionData->GetConnectionId(), static_cast<Module::grade_t>(handlerData->grade))) {
