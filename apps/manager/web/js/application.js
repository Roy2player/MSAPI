...
class View {
	...
	FindClosestToCling({ top, right, bottom, left })
	{
		...
		for (const other of View.GetCreatedApplications().values()) {
	...
	static SaveApplication(app)
	{
		...
		View.#privateFields.m_lastCreatedApplication = app;
		View.#privateFields.m_createdApplications.set(app.m_uid, app);
	}

	static GetLastCreatedApplication() { return View.#privateFields.m_lastCreatedApplication; }

	static GetCreatedApplications() { return View.#privateFields.m_createdApplications; }

	static GetApplicationByPort(port)
	{
		for (let app of View.#privateFields.m_createdApplications.values()) {
			if (app.m_port == port) {
				return app;
			}
		}
	...
	static RemoveCreatedApplication(app) { View.#privateFields.m_createdApplications.delete(app.m_uid); }
	...
	constructor(viewType, parameters)
	{
		(async () => {
			try {
				...
				View.SaveApplication(this);
				this.m_parentView.style.zIndex = View.#privateFields.m_createdApplications.size;
				...
				function StartResizing(element, direction, savedThis)
				{
					...
					View.GetCreatedApplications().values().forEach(
	...
	async Constructor(viewType, parameters)
	{
		...
		if (viewType === "ModifyApp") {
			let existedApp = View.GetApplicationByPort(parameters.port);
			if (existedApp) {
				existedApp.Show();
				return false;
			}
		...
		if (viewType === "CreatedApps") {
			...
			this.AddCallback("delete", (response, extraParameters) => {
				...
				for (let app of View.GetCreatedApplications().values()) {
	...
	Destructor()
	{
		View.RemoveCreatedApplication(this);
		this.Show();
	...
	MakeDraggable(node, element)
	{
		...
		node.addEventListener("mousedown", (e) => {
			...
			View.GetCreatedApplications().values().forEach((app) => { app.m_view.classList.add("changing"); });
	...
	{
		if (app.m_parentView.style.zIndex == View.#privateFields.m_createdApplications.size) {
			return;
		}

		const shiftedPosition = app.m_parentView.style.zIndex;

		View.#privateFields.m_createdApplications.forEach((item, uid) => {
			if (uid != app.m_uid) {
				...
				item.m_parentView.style.zIndex = View.#privateFields.m_createdApplications.size;
			}
		});
	}
}
